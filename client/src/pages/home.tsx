...
async function createSession(): Promise<Memory> {
...
async function generateMemory(sessionId: string): Promise<Memory> {
  const response = await fetch(`/api/memories/${sessionId}/generate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  });
...
async function updateMemory(sessionId: string, memory: string): Promise<Memory> {
  const response = await fetch(`/api/memories/${sessionId}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
export default function Home() {
  ...
  const selectedMemory = selectedSessionId
    ? allMemories.find((m) => m.sessionId === selectedSessionId)
  ...
  const generateMutation = useMutation({
    mutationFn: async () => {
      ...
      // Generate memory for it
      const memory = await generateMemory(session.sessionId);
      // Set it as selected
  ...
  const updateMutation = useMutation({
    mutationFn: (memory: string) => {
      ...
      return updateMemory(selectedSessionId, memory);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["memories"] });
    },
      ...
      >
        <div className="container flex h-16 items-center justify-between px-4 sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold leading-none">Memory Generator</h1>
                      ...
                      {allMemories.map((memory) => (
                        <tr
                          ...
                          className={`border-b border-border hover:bg-muted/50 cursor-pointer transition-colors ${
                            selectedSessionId === memory.sessionId ? "bg-muted" : ""
                          }`}
                          onClick={() => setSelectedSessionId(memory.sessionId)}
                        >
                          ...
                          <td className="py-2 px-3">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                setSelectedSessionId(memory.sessionId);
                              }}
                            >
                              Select
                            </Button>
